...
A separate Python backend is required because the portal has no API, so scraping must run
...
- All traffic is HTTPS. Not affiliated with SRM, so use at your own risk.
