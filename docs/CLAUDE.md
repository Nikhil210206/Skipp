...
### DONE: Saturday, which the portal does not publish (2026-08-29)

Reported as students having Saturday classes that academia never shows, and
wanting to add them by hand. Checked before building: **all 26 Saturdays in the
real captured planner carry `dayOrder: null`**, holidays included. The portal is
not hiding them, it genuinely has none.

**THE WHOLE DESIGN IS ONE DECISION: A SATURDAY IS NOT A DAY ORDER.** The
obvious build is a sixth entry in the day-order grid, and it is wrong. Every
piece of attendance arithmetic in this app is gated on `dayOrder != null`: the
leave predictor, `focusDay`, the term-progress count, the long-weekend runs in
`lib/holidays.ts`. A synthetic day order changes all of them at once, silently,
for a class the portal does not track.

So `lib/saturday.ts` is a **separate store under a separate key**
(`skipp.saturday.<reg>`), never merged into `timetable.dayOrders`. Day-order
code cannot read it even by accident, which is a structural guarantee rather
than a rule somebody has to remember. Verified live: Schedule still shows
exactly five day-order tabs with no Saturday in them, no calendar cell carries
both a Saturday ring and a day order, and the leave planner offers **one**
selectable date in August (the 31st, a Monday) with every Saturday refused.

**The consequence is real and is stated in the UI rather than discovered.**
These classes carry a name the student typed, not a course code, so nothing
here can be charged to a subject. A Saturday you skip does not appear in the
leave planner. That is what "free text" costs, and the sheet says so.

**THE SATURDAY BATCH IS NOT THE PORTAL'S BATCH.** They use the same two
numbers and mean different things: `StudentInfo.batch` is the ordinary week,
the Saturday split is its own arrangement (batch 1 the morning, batch 2 the
afternoon). Seeding one from the other was built first and taken back out on
request. It looks like a helpful default and is really **a wrong answer
presented confidently**, which is worse than no answer, because a student has
to notice it is wrong before they can fix it. The batch is `null` until they
say, the sheet asks, and a test asserts the store never mentions
`student.batch` at all.

For the same reason the suggested period times are **not** read off the weekday
grid. That grid's bells (08:50, 09:45, 01:25) belong to the ordinary week. Four
one-hour slots across the batch's own window, and the student can type anything
inside it.

**The batch is what removes the AM/PM control.** Batch 1 sits in the morning
and batch 2 in the afternoon, so a student who has said which one they are in
has already answered which half of the day it is. Asking twice only lets them
disagree with themselves, and a class filed at 2 in the morning is the sort of
thing nobody notices until their Saturday is empty. The window then bounds the
input: a batch 2 student typing 9am is refused with "Batch 2 runs 1pm to 5pm."

**The entry point is a date, not a tab.** Tapping a Saturday on Calendar. That
is not merely where it was asked for, it is the only honest place: Schedule is
the day-order screen, so a sixth tab there would assert a day order that does
not exist, while a Saturday only exists as a date.

**In the grid a Saturday with classes is a RING, and shape is doing the work.**
Filled green is already the holiday and the accent already means two things on
this grid (the selection, today's underline), so a third use of either would
leave all of them meaning nothing. The numeral goes to `text-2`: clear of a
dead weekend, below a working day's `text-1`, which is exactly its rank. And
**a holiday still wins outright**, verified on 15 August 2026, which is both a
Saturday and Independence Day: it keeps the dot and gets no ring.

**HOME IS WHERE THE FEATURE ACTUALLY PAYS OFF, and it needed four conditions,
each of which is a bug without it.** `focusDay` walks past a Saturday to
Monday, so without this a student is told on Friday night AND on Saturday
morning that their next class is two days away while they are on their way to
one. `saturdayFocus()` is pure and lives in the lib precisely so it can be
tested, and it returns a `FocusDay` whose `dayOrder` is null and stays null
(the cover's giant ghost numeral is gated on that, so a Saturday prints no
numeral, which is the truth rather than a gap):

- **Still to come.** Once the last class ends the day is over. Without it the
  cover counts down to a time already past.
- **Not later than the portal's answer.** A Tuesday with classes left keeps
  Tuesday. A Friday evening gets the Saturday rather than Monday.
- **Inside the term.** A stored plan outlives the semester it was typed for.
- **Not a holiday, and not a day the portal claims.** If a term ever declares a
  working Saturday it will carry a day order, and then the portal is right and
  this list is not.

**Notifications needed the same door.** `NotifyOnOpen` bailed on
`today?.dayOrder == null`, so the one student with a Saturday class was the one
the reminder never reached.

**`TextField` and the clock inputs moved to `components/ui/fields.tsx`** the
moment a second sheet wanted them. Everything hard-won about them only gets
fixed once if it only exists once: the whole box being a tap target, the caret
advancing when the hour cannot grow, and **reading the blur off the element
rather than the prop**. `ClockField` carries no AM/PM; `CustomClassSheet` pairs
it with a Segmented because a custom class can land anywhere in the day.

**A fixture bug was found doing this and is worth the warning.**
`lib/__devFixture.ts` marked only Sunday as off, so its Saturdays carried day
orders and **the fixture could not express the situation this feature exists
for**. The first run against it showed "Day order 2" on a Saturday and no
Saturday UI at all. Fixed to match the real planner. **A fixture that cannot
represent the bug is worse than no fixture**, because it reports success.

**DAYS OFF ARE TOLD FROM THE STUDENT'S OWN WEEK (2026-08-29, later).** Asked
for directly: Saturdays should not count as days off unless the portal itself
marks that Saturday a holiday. **A break is only a break if you do not have to
come back into the middle of it**, so for a student who sits Saturday classes,
Gandhi Jayanthi on a Friday is one day off and not three.

**Per student, not universal, and that was the decision worth taking.** Against
the real planner the change is large: Krishna Jayanthi 3 to 1, Gandhi Jayanthi
3 to 1, Vinayakar Chathurthi 3 to 2, Ayutha Pooja 4 to 3, Diwali 2 to 1, and
**long weekends 4 down to 1**. Applying that to everybody would tell a student
with no Saturday classes that Gandhi Jayanthi is one day off when they
genuinely get three. `termHolidays` takes `{ saturdaysAreClassDays }`, and left
off it is byte for byte what it always was, which is asserted.

**This is the ONE place Saturday reaches outside its own store, deliberately.**
The dependency runs one way (holidays asks about Saturday, never the reverse),
it asks only about DATES and never reads the store, and it reaches a displayed
figure: nothing here touches attendance, the day-order grid or the leave
planner. The import test now asserts that shape rather than a blanket ban.

**A naive version of this breaks `resumes`, and it is not obvious.** That field
tested `next.dayOrder != null`, and a working Saturday carries no day order, so
a Friday holiday followed by a working Saturday came back as **"after the
term"**. One `isClassDay(d)` now serves the run walk and `resumes` both, so
they cannot disagree.

**A festival on a Saturday is a real day gained**, so the "falls on a weekend"
caveat had to stop being about what the day is CALLED. `Holiday.onFreeWeekend`
replaces the old `isWeekendDay(h.weekday)` test: Sunday is free for everybody,
Saturday only for a student who does not sit classes on it. Independence Day
2026 is a Saturday and portal-marked, and for a Saturday student it now reads
as an ordinary day off instead of telling them they gained nothing.

**Term progress was deliberately left alone.** It still counts day orders only,
so it keeps meaning "working days the portal knows about", which is what
attendance is measured against.

**AND IT EXPOSED A REAL BUG ONE LEVEL DEEPER: THE PLANNER IS WIDER THAN THE
TERM.** Christmas came back as `resumes: true`, because the 26th of December is
a Saturday with no day order and the predicate happily called it a class day.
The real planner runs **1 July to 31 December while teaching runs 21 July to 7
December**, so it carries three Saturdays before classes start, three after
they stop, and a 24 day tail with no day orders at all. Every one of those was
being treated as an ordinary Saturday: **rings on the calendar grid, a
countdown on Home and a notification, for classes nobody was holding.**

`teachingPeriod()` is the fix, and **being IN the calendar is not the same as
being in the term** is the lesson. Of 26 Saturdays, 19 hold classes and 7 do
not: three before the first working day, three after the last, and Independence
Day. This is the same shape as the notification guard below, one level in, and
it was found by a holiday test rather than by looking for it.

**FOUR BUGS FOUND IN A DELIBERATE HUNT AFTERWARDS, and the first is the one
that mattered.**

1. **The notification path announced Saturday classes for ever.** Its guard was
   written inline as `today?.dayOrder == null`, and **that is TRUE for a date
   the calendar has never heard of**, so once the term ended a student would
   have gone on being told a Saturday class was starting, every weekend,
   indefinitely. Home refused those same dates correctly, which is what made it
   findable: the two were asked to answer the same question and disagreed. The
   guard had been spelled out by hand in three places in three slightly
   different ways. **`isSaturdayClassDay()` is now the one predicate** and all
   of them call it, so they cannot drift again. Asserted directly: Home and the
   notifications now agree on all 184 days of the term.
2. **The Calendar had the same hole**, from the same duplication: a ring could
   have been drawn, and the day panel opened, on a Saturday outside the term.
3. **Changing batch stranded classes silently.** Type a Saturday as batch 2,
   switch to batch 1, and four afternoon classes sit there that the app would
   refuse if they were typed today. They are now shown back marked "Outside
   batch 1" with a line saying they are kept as typed. **Nothing is deleted**:
   answering a correction by destroying work is how somebody loses a Saturday
   they spent five minutes entering.
4. **The sheet kept a stale complaint across a reopen.** It stays mounted, so a
   half-typed class and the error about it were both still sitting there next
   time, contradicting a form the student had not touched. The same trap
   `CustomClassSheet` had already documented and fixed.

**A FIFTH BUG WAS FOUND THAT HAS NOTHING TO DO WITH SATURDAY**, while checking
the console on a screen this work does not touch. `attendance/page.tsx` keyed
its rows `code-slot`, and **a course has separate Theory and Practical rows
that can share both**, so React saw duplicate keys (`21CSC302J-A`) and is free
to drop one: a whole subject silently missing from the screen the app exists
for. The category is part of the key now. Live with the fixture: 8 of 8 rows,
both Theory/Practical pairs intact.

**A TEST THAT PASSED VACUOUSLY, recorded because the technique is what caught
it.** The first non-interference assertions read `projection.after`, and the
field is `overallAfter`, so they compared `undefined === undefined` and passed
while proving nothing. **The control caught it**: a deliberate assertion that
one ordinary weekday off DOES move the number failed with `undefined vs
undefined`, which is the only reason anyone looked. **Every non-interference
test needs a control that must fail.**

That control then exposed something worth understanding rather than fixing:
selecting Saturdays does move the projected percentage, and **it is not
Saturday leaking in**. Picking any far-off date extends the projection window,
so the student is credited with everything they would attend in between. The
proof is that **the Sunday beside each Saturday gives an identical answer**:
both are simply days with no day order. Meanwhile `affectedDays` and
`totalMissed` are zero, so no Saturday class is ever charged to anybody.

**THE STRONGEST GUARANTEE IS AN IMPORT-GRAPH TEST, not a behavioural one.**
Fourteen modules are asserted never to import `lib/saturday`: the leave
predictor, the predictor, schedule, reminders, grades, holidays, the timetable
image, portal attendance, `PredictModal`, `CustomClassSheet`, and the
timetable, attendance, marks and profile screens. And the reverse: exactly five
files may know Saturday exists, `lib/saturday` imports nothing but schedule
types and the shared types, it never mentions `student.batch`, and the only day
order it ever writes is `null`. **A behavioural test proves today; an import
test prevents tomorrow.**

Verified live with two Saturday classes actually stored under a marker string
no fixture course could match: the marker appears on Home and Calendar and on
**no other screen**, not Marks, Attendance, Schedule or Profile, and on none of
the five day-order tabs. The leave planner was swept across four months: 64
selectable dates, **zero of them Saturdays**. The timetable PNG export is proved
structurally instead of by reading pixels, since its call site takes
`attendingDayOrders` and `customClasses` and nothing else.

**Verified.** 30 assertions against the real `lib/saturday.ts` (jiti, stubbed
localStorage, no browser and no portal sign-in) covering the windows, the
suggestions, the store round trip, junk and legacy values, ordering and the
display adapter. Then a 47 assertion non-interference suite **against the real captured
planner**, covering the leave planner, the day-order helpers, the import graph,
every boundary of the predicate and the batch window, including a sweep of all
184 days asserting that nothing featured ever carries a day order, is ever not
a Saturday, or is ever a holiday. Then
the whole loop in a browser on a 375px phone through `?fixture=1`, which is
what caught the fixture bug and cost no sign-in.
